...
const Zotero_RTFScan = new function() {
    ...
    const localeService = Components.classes["@mozilla.org/intl/nslocaleservice;1"].
        getService(Components.interfaces.nsILocaleService);
    const appLocale = localeService.getApplicationLocale();

    const _localizedStringBundle = stringBundleService.createBundle(
    ...
    this.introPageShowing = function() {
        const fileType = Zotero.Prefs.get("ODFScan.fileType");
        const outputMode = Zotero.Prefs.get("ODFScan.outputMode");
        let mode_string = [fileType];
        if (outputMode) {
            mode_string.push(outputMode);
        }
        mode_string = mode_string.join("-");
        const selectedNode = document.getElementById("file-type-selector-" + mode_string);
    ...
    this.chooseInputFile = function() {
        ...
        const outputMode = Zotero.Prefs.get("ODFScan.outputMode");
        const inputPath = Zotero.Prefs.get("ODFScan."+fileType+".lastInputFile" + outputMode);
        if (inputPath) {
            if (!inputFile) {
    ...
    this.chooseOutputFile = function() {
        ...
        if (inputFile) {
            ...
            const suffix = (" " + _getString("ODFScan."+fileType+".scannedFileSuffix" + outputMode));
            if (fileType === "odf") {
                const suffixMatchers = [ " " + _getString("ODFScan.odf.scannedFileSuffixtomarkers"), " " + _getString("ODFScan.odf.scannedFileSuffixtocitations") ];
        ...
        // Set directory if possible
        const outputPath = Zotero.Prefs.get("ODFScan."+fileType+".lastOutputFile" + outputMode);
        if (outputPath) {
            if (!outputFile) {
    ...
    function _scanODF(outputMode) {
        ...
        const tmplCitation = "<text:reference-mark-start text:name=\"ZOTERO_ITEM {&quot;properties&quot;:{&quot;formattedCitation&quot;:&quot;%{1}s&quot;},&quot;citationItems&quot;:%{2}s} RND%{3}s\"/>%{4}s<text:reference-mark-end text:name=\"ZOTERO_ITEM {&quot;properties&quot;:{&quot;formattedCitation&quot;:&quot;%{5}s&quot;},&quot;citationItems&quot;:%{6}s} RND%{7}s\"/>";
        const tmplText = "{ %{1}s | %{2}s | %{3}s | %{4}s |%{5}s}";

        const rexPref = /<meta:user-defined meta:name="ZOTERO_PREF[^<]*?<\/meta:user-defined>/;
        const rexLabels = /^((?:art|ch|Ch|subch|col|fig|l|n|no|op|p|pp|para|subpara|pt|r|sec|subsec|Sec|sv|sch|tit|vrs|vol)\\.)\\s+(.*)/;
        const rexBalancedTags = /(.*)<([-:a-zA-Z0-9]*)[^\/>]*>([^<]*)<\/([-:a-zA-Z0-9]*)[^>]*>(.*)/;
        const rexLink = /(<[^>]*xlink:href=\"([^\"]*)\"[^>]*>)\s*{([^\|{}]*)\|([^\|}]*)\|([^\|}]*)\|([^\|}]*)}\s*(<[^>]*>)/;
        const rexLink2 = /(<[^>]*xlink:href=\"([^\"]*)\"[^>]*>)\s*(?:<[^\/>]+>)\s*{([^\|{}]*)\|([^\|}]*)\|([^\|}]*)\|([^\|}]*)}\s*(?:<\/[^\/>]+>)\s*(<[^>]*>)/;
        const rexNativeLink = /<text:reference-mark-start[^>]*ZOTERO_ITEM\s+(?:CSL_CITATION\s+)*([^>]*)\s+[^ ]*\/>(.*?)<text:reference-mark-end[^>]*\/>/;
        const checkStringRex = /(<[^\/>][^>]*>)*{[^<>\|]*|[^<>\|]*|[^<>\|]*|[^<>\|]*|[^<>\|]*}(<\/[^>]*>)*/;
        const openTagSplitter = /(<[^\/>][^>]*>)/;
        const closeTagSplitter = /(<\/[^>]*>)/;
        const rexSingleton = /<[^>]*\/>/g;
        const rexSpace = /<text:s\/>/g;
        const rexPlainTextLinks = /({[^\|{}]*\|[^\|}]*\|[^\|}]*\|[^\|}]*\|[^\|}]*})/;
        const rexWrappedLinks = /(<[^>]*xlink:href=\"[^\"]*\"[^>]*>\s*(?:<[^\/>]+>)?\s*{[^\|{}]*\|[^\|}]*\|[^\|}]*\|[^\|}]*}\s*(?:<\/[^\/>]+>)?\s*<[^>]*>)/;
        const rexNativeLinks = /(<text:reference-mark-start[^>]*ZOTERO_ITEM\s+(?:CSL_CITATION\s+)*[^>]*\/>.*?<text:reference-mark-end[^>]*\/>)/;
        const rexCite = /({[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*})/;
        const rexCiteExtended = /(<\/?text:span[^>]*>{[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*}<\/?text:span[^>]*>)/;
        const rexCiteExtendedParts = /(<text:span[^>]*>)({[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*})(<\/text:span>)/;
        const rexCiteExtendedPartsReverse = /(<\/text:span>)({[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*\|[^<>\|]*})(<text:span[^>]*>)/;

        const rexFixMarkupBold = /[\*][\*](.*?)[\*][\*]/;
        const rexFixMarkupItalic = /\*(.*?)\*/;

        const rexTextAll = /{\s*([^|{}]*)\|\s*([^|}]*)\s*\|\s*([^|}]*)\s*\|([^|}]*?)\s*\|\s*([^|}]*)\s*}/g;
        const rexText = /{\s*([^|{}]*)\|\s*([^|}]*)\s*\|\s*([^|}]*)\s*\|([^|}]*?)\s*\|\s*([^|}]*)\s*}/;
        const rexTextPlain = /{[^|{}]*\|[^|}]*\|[^|}]*\|[^|}]*\|[^|}]*}/;
        const rexEmptyBalanceSpan = /^<text:span[^>]*><\/text:span[^>]*>$/;
        ...
        Fragment.prototype.removeBalancedTags = function (str) {
            while (true) {
                const m = str.match(rexBalancedTags);
                if (m) {
                    if (m[2] === m[4]) {
        ...
        Fragment.prototype.normalizeNativeMarks = function () {
            ...
            const m = this.newtxt.match(rexNativeLink);
            if (m) {
                const m_citation = m[1];
                let m_plaintext = this.removeBalancedTags(m[2]);
                let replacement = "";
                const obj_txt = m_citation.split("&quot;").join("\"");
                const obj = JSON.parse(obj_txt);
                let count = 1;
                for (let i=0,ilen=obj.citationItems.length;i<ilen;i+=1) {
                    const item = obj.citationItems[i];
                    ...
                    if ("undefined" !== typeof item.label && item.locator) {
                        const mm = item.locator.match(rexLabels);
                        if (!mm) {
        ...
        Fragment.prototype.finalize = function (msg) {
            const m = this.newtxt.match(checkStringRex);
            if (m) {
        ...
        ODFConv.prototype.composeCitations = function () {
            ...
            const m = [];
            const m_all = this.content.match(rexTextAll);
            for (let i=0,ilen=m_all.length;i<ilen;i+=1) {
                const subm = [];
                const m_one = m_all[i].match(rexText);
                for (let j=1,jlen=m_one.length;j<jlen;j+=1) {
                    subm.push(m_one[j]);
                }
                m.push(subm);
            }
            const lst = this.content.split(rexTextPlain);
            ...
            for (let i=m.length-1;i>-1;i+=-1) {
                ...
                const link = this.fixMarkup(m[i][4]).replace(/^\s+/,"").replace(/\s+$/,"");
                ...
                const myid = link.slice(params.offset);
                const myidlst = myid.split(params.splitter);
                if (myidlst.length === 2) {
                ...
                if (lst[i]) {
                    let placeholder = placeholder.join("; ")
                        .split("\"").join("")
                        .split(/[\\]*&quot;/).join("");
                    const escapedPlaceholder = placeholder.split("&").join("&amp;");
                    let items = JSON.stringify(items);
                    items = items.split("\\\\&quot;").join("\\&quot;");
                    items = items.split("\"").join("&quot;");
                    const randstr = this.generateRandomString();
                    const citation = tmplCitation.replace("%{1}s", escapedPlaceholder)
                        .replace("%{2}s",items)
                        .replace("%{3}s",randstr)
        ...
        ODFConv.prototype.generateRandomString = function (str) {
            let randstr;
            while (true) {
                const nums = [49,50,51,52,53,54,55,56,57,65,66,67,68,69,70,71,72,73,74,75,76,77,78,79,80,81,82,83,84,85,86,87,88,89,90,97,98,99,100,101,102,103,104,105,106,107,108,109,110,111,112,113,114,115,116,117,118,119,120,121,122];
                randstr = "";
        ...
        ODFConv.prototype.readZipfileContent = function () {
            ...
            function _getEntryContent(fileName) {
                const inputStream = zipReader.getInputStream(fileName);
        ...
        ODFConv.prototype.purgeStyles = function () {
            ...
            const decodeXML = Components.classes["@mozilla.org/xmlextras/domparser;1"]
                .createInstance(Components.interfaces.nsIDOMParser);
            const encodeXML = new XMLSerializer();

            const doc = decodeXML.parseFromString(this.content,"application/xml");
            const noteBodies = doc.getElementsByTagName("text:note-body");
            const stylesUsedInNotes = {};
            ...
            function inspectNote(node) {
                if (node.hasAttribute("text:style-name")) {
                    const styleName = node.getAttribute("text:style-name").toString();
            ...
            function fixStyleNodes() {
                ...
                for (let i=0,ilen=styleNodes.length;i<ilen;i++) {
                    ...
                    const styleName = styleNode.getAttribute("style:name").toString();
            ...
            function fixStyleNode(node) {
                const textPropertyNodes = node.getElementsByTagName("style:text-properties");
                for (let i=0,ilen=textPropertyNodes.length;i<ilen;i++) {
                    const textPropertyNode = textPropertyNodes[i];
                    const unwantedAttributes = ["style:font-name","fo:font-size","style:font-size-asian"];
                    for (let j=0,jlen=unwantedAttributes.length;j<jlen;j++) {
                        const attributeName = unwantedAttributes[j];
    ...
    function _scanRTF() {
        ...
        while ((m = citationRe.exec(contents))) {
            ...
            if (lastCitation && lastCitation.end >= start) {
                ...
                const lastCitation = {"citationStrings":[citationString], "pages":[pages], "start":start,
            ...
            for (let i=0; i<creators.length; i++) {
                ...
                const lastName = spaceIndex == -1 ? creators[i] : creators[i].substr(spaceIndex+1);
            ...
            if (!ids) {    // no mapping found
                ...
                if (items.length == 1) {    // only one mapping
                    ...
                    const treeitem = _generateItem(citationString, "");

                    // generate child items
                    const treeitemChildren = document.createElement("treechildren");
    ...
    function _matchesItemCreator(creator, itemCreator) {
        ...
        const lowerLast = itemCreator.ref.lastName.toLowerCase();
        ...
        if (creator.length > lowerLast.length) {
            ...
            if (firstName.length) {
                ...
                const initialRe = /^(?:[A-Z]\.? ?)+$/;
                const m = initialRe.exec(firstName);
                if (m) {
                    const initials = firstName.replace(/[^A-Z]/g, "");
                    const itemInitials = itemCreator.ref.firstName.split(/ +/g).map(function(name){
                        return name[0].toUpperCase();
                    }).join("");
                    if (initials != itemInitials) return false;
                } else {
                    // not all initials; verify that the first name matches
                    const firstWord = firstName.substr(0, itemCreator.ref.firstName).toLowerCase();
                    const itemFirstWord = itemCreator.ref.firstName.substr(0, itemCreator.ref.firstName.indexOf(" ")).toLowerCase();
    ...
    this.treeClick = function(event) {
        ...
        row = row.value;
        const level = tree.view.getLevel(row);
        if (col.value.index == 2 && level > 0) {
            const iconColumn = col.value;
            const itemNameColumn = iconColumn.getPrevious();
            const citationColumn = itemNameColumn.getPrevious();

            if (level == 2) {        // ambiguous citation item
                // get relevant information
                const parentIndex = tree.view.getParentIndex(row);
                ...
                const citation = tree.view.getCellText(row, citationColumn);
                const io = {singleSelection:true};
                ...
                if (io.dataOut && io.dataOut.length) {
                    const selectedItemID = io.dataOut[0];
                    const selectedItem = Zotero.Items.get(selectedItemID);

                    const treeitem = tree.view.getItemAtIndex(row);

                    // remove any children (if ambiguous)
                    const children = treeitem.getElementsByTagName("treechildren");
    ...
    function _refreshCanAdvance() {
        ...
        for (let i=0,ilen=citationItemIDs.length;i<ilen;i++) {
            const itemList = citationItemIDs[i];
    ...
    this.fileTypeSwitch = function (mode) {
        ...
        for (let i=0,ilen=nodeIdStubs.length;i<ilen;i+=1) {
            const elems = document.getElementsByClassName(nodeIdStubs[i]);
    ...
    function _formatRTF() {
        ...
        const zStyle = Zotero.Styles.get(document.getElementById("style-listbox").selectedItem.value);
        const style = zStyle.getCiteProc();
        style.setOutputFormat("rtf");
        const isNote = style.class == "note";

        // create citations
        const k = 0;
        const cslCitations = [];
        let itemIDs = {};
        const shouldBeSubsequent = {};
        for (let i=0; i<citations.length; i++) {
            const citation = citations[i];
            const cslCitation = {"citationItems":[], "properties":{}};
            if (isNote) {
        ...
        // TODO: itemIDs is an object, how does this work?
        ...
        const citationResults = style.rebuildProcessorState(cslCitations, "rtf");
        ...
        if (zStyle.hasBibliography) {
            ...
            if (contents.indexOf(BIBLIOGRAPHY_PLACEHOLDER) !== -1) {
                ...
                const bracketRe = /^\{+/;
                const m = bracketRe.exec(contents);
                if (m) {
                    const closeBracketRe = new RegExp("(\\}{"+m[0].length+"}\\s*)$");
